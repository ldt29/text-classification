...
from dataset import CLSDataset, BOWDataset, FastTextDataset, collate_fn, collate_fn_fasttext
import numpy as np
import collections
...
def train(args):
    ...
    if args.model_type == 'bow':
        ...
        train_dataset = FastTextDataset(dictionary=dictionary, split='train', n=2);
        valid_dataset = FastTextDataset(dictionary=dictionary, split='valid', n=2);
        test_dataset = FastTextDataset(dictionary=dictionary, split='test', n=2);

        train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn_fasttext)
        valid_loader = DataLoader(valid_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_fasttext)
        test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_fasttext)
